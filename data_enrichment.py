...
# --- Step 1: Load your combined MLS datasets ---
print("Loading local MLS datasets...")
# Using low_memory=False to handle large datasets (300MB+) efficiently
...
# Convert specific dates to a 'Year-Month' period (e.g., 2024-01)
...
print("Creating time-series join keys...")
...
# Checking for nulls ensures every transaction was matched with a mortgage rate
null_sold = sold_enriched['rate_30yr_fixed'].isnull().sum()
null_listings = listings_enriched['rate_30yr_fixed'].isnull().sum()

print(f"\n--- Validation Results ---")
print(f"Missing rates in Sold dataset: {null_sold}")
print(f"Missing rates in Listings dataset: {null_listings}")

# Preview the results to confirm 'rate_30yr_fixed' is correctly attached
print("\nPreview of Enriched Sold Data:")
print(sold_enriched[['CloseDate', 'year_month', 'ClosePrice', 'rate_30yr_fixed']].head())

# --- Step 7: Save the enriched datasets ---
# These files will be large; remember to keep them in your .gitignore
sold_enriched.to_csv('CRMLSSold_Enriched.csv', index=False)
listings_enriched.to_csv('CRMLSListing_Enriched.csv', index=False)
print("\nSuccess! Enriched datasets saved as new CSV files.")